...
for page in report:
    for data in page:
        simplified_node = {
            ...
            # Use .get in case some nodes don't have a parent_id field
            'parent_id': data.get('parent_id', ''),
            'children_ids': data.get('children_ids', []),
            'children_nodes': []}
        nodes_dict[data['self_id']] = simplified_node

root = nodes_dict['r0']

for node_id, node in nodes_dict.items():
    if node_id != 'r0' and node['parent_id'] in nodes_dict:
        parent_id = node['parent_id']
        nodes_dict[parent_id]['children_nodes'].append(node)
