                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap' }}>
                        <div style={{ flex: '1 0 0 ', margin: '0 10px 10px 0' }}>
                            <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '5px', border: '1.25px solid #ccc', borderRadius: '8px', boxShadow: '2px 2px 8px rgba(0,0,0,0.1)', padding: '10px' }}>
                                {Object.keys(sentimentColors).map(sentiment => (
                                    <div key={sentiment} style={{ marginLeft: '15px' }}>
                                        ...
                                        />
                                        <label htmlFor={sentiment} style={{ marginLeft: '7px', fontSize: '15px' }}>{sentiment.charAt(0).toUpperCase() + sentiment.slice(1)}</label>
                                    </div>
                                ))}
                            </div>
                            ...
                            >
                                ...
                                <XAxis dataKey="index">
                                    <Label value="Index" offset={-15} position="insideBottom" style={{fontSize: '20px'}} />
                                </XAxis>
                                <YAxis>
                                    <Label value="Score" angle={-90} position="insideLeft" style={{fontSize: '20px'}}/>
                                </YAxis>
                                <Tooltip content={<CustomTooltip />} />
                                <Legend verticalAlign="top" wrapperStyle={{ lineHeight: '40px', fontSize: '20px' }} />
                                ...
                                <Brush y={580} height={55} stroke="#8884d8" /> {/* The y value might need to be adjusted */}
                            </LineChart>
                            </ResponsiveContainer>
                        </div>
